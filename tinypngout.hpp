...
private:
    std::uint32_t positionX; // Next byte index in current line
private:
    std::uint32_t positionY; // Line index of next byte
private:
    std::uint32_t uncompRemain; // Number of uncompressed bytes remaining
private:
    std::uint16_t deflateFilled; // Bytes filled in the current block (0 <= n < DEFLATE_MAX_BLOCK_SIZE)
private:
    std::uint32_t crc; // Primarily for IDAT chunk
private:
    std::uint32_t adler; // For DEFLATE data within IDAT
     ...
     */
public:
    explicit TinyPngOut(std::uint32_t w, std::uint32_t h, std::ostream &out);
...
private:
    static constexpr std::uint16_t DEFLATE_MAX_BLOCK_SIZE = UINT16_C(65535);
};